...
public class UserManagement
{
	...
	public void FileToObject()
	{
		...
		try
		{
			in = new Scanner(Paths.get("User_database1.txt"));
		}
		catch(IOException e)
		{
			error = true;
			throw new NullPointerException("Can't find the file");
		}
		if(!error)
		{
			while(in.hasNext())
			{
            	...
            	{
            		throw new NullPointerException("Can't parse the date");
              	} 
	...
	{
		try{
			Formatter out = new Formatter("User_database1.txt");
			for(User i : ArrayListinput)
			{
				out.format("%s", i.getData());
				out.format("\n");
			}
			out.close();
		}
