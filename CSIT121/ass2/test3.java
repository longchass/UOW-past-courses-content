...
public static void main (String[] args)
{
			...
			login.FileToObject();
			ServiceManagement serv = new ServiceManagement();
			serv.FileToObject();
			BookingManagement book = new BookingManagement();
			book.FileToObject(login, serv);
			...
			System.out.println("6: search a service by name");
			if(loggedinUser.getUserType() == UserType.Staff)
			{
				...
				System.out.println("7: Search booking record using username");
			}
			...
			else if(options == 6)
			{
				...
				System.out.println(serv.findServiceUsingKeywordOnName(keyword));

			}
			else if(options == 7 && loggedinUser.getUserType() == UserType.Staff)
			{
				String keyword = input.nextLine();
				for(BookingRecord i : book.findBookingRecordByUserName(keyword))
				{
					System.out.println(i);
				}
			}
